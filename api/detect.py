...
from utils.general import (
    cg_size, non_max_suppression, apply_classifier, scale_coords,
    xh, plot_one_box, strip_optimizer, set_logging)
...
def detect(config, save_img=False):
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        # Apply NMS
        pred = non_max_suppression(pred, config['conf_thres'], config['iou_thres'], classes=config['classes'], agnostic=config['agnostic_nms'])
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        if config['update']:  # update all models (to fix SourceChangeWarning)
            for config['weights'] in ['yolov5s.pt', 'yolov5m.pt', 'yolov5l.pt', 'yolov5x.pt']:
                detect(config)
